...
function decrypt_key(deploy_key, enc_rsa_key_pth, options) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    // config/deploy_id_rsa_enc => config/deploy_id_rsa
    // config/deploy_id_ed_ed25519_enc => config/deploy_id_ed25519
    const public_key_pth = enc_rsa_key_pth.replace("_enc", "")
    const runner = new toolrunner.ToolRunner('openssl',
        ['enc', '-d', '-aes-256-cbc', '-md', 'sha512', '-salt', '-in',
         enc_rsa_key_pth, '-out', public_key_pth, '-k', deploy_key, '-a'], options);
    yield runner.exec();

    const runner1 = new toolrunner.ToolRunner('chmod', ['0600', public_key_pth], options);
    yield runner1.exec();
    ...
    const runner3 = new toolrunner.ToolRunner('ssh-add', [public_key_pth], options);
    yield runner3.exec();
  });
}
